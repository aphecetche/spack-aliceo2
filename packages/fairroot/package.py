...
class Fairroot(CMakePackage):
    ...
    patch('support_geant4_with_external_clhep.patch',
          when='@18.4 +sim ^Geant4@:10.5')
    ...
    patch('apple-clang-15.patch', when='%apple-clang@15:')
    ...
    def cmake_args(self):
        ...
        if self.spec.satisfies('@:18,develop'):
            ...
            if '+sim' in self.spec:
                options.append(
                    '-DPYTHIA8_DIR={0}'.format(self.spec['pythia8'].prefix))
